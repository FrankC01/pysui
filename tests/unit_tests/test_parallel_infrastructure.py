...
# ===========================================================================
# UnResolvingArgParser — GQL
# ===========================================================================

class TestUnResolvingArgParserGQL:
    """Test the fixed UnResolvingArgParser for GQL."""

    @pytest.fixture
    def parser(self):
        from pysui.sui.sui_pgql.pgql_txn_async_argb import UnResolvingArgParser
        client = MagicMock()
        return UnResolvingArgParser(client)

    @pytest.mark.asyncio
    async def test_str_arg_no_expected_type(self, parser):
        result = await parser.fetch_or_transpose_object(
            "0xdeadbeef", is_receiving=False, is_mutable=False, expected_type=None
        )
        assert isinstance(result, bcs.UnresolvedObjectArg)
        assert result.ObjectStr == "0xdeadbeef"
        assert result.IsReceiving is False
        assert result.RefType == 0  # NO_REF

    @pytest.mark.asyncio
    async def test_mutable_ref_type(self, parser):
        result = await parser.fetch_or_transpose_object(
            "0xdeadbeef", is_receiving=False, is_mutable=True, expected_type=None
        )
        assert result.RefType == 2  # MUT_REF

    @pytest.mark.asyncio
    async def test_receiving_flag(self, parser):
        result = await parser.fetch_or_transpose_object(
            "0xdeadbeef", is_receiving=True, is_mutable=False, expected_type=None
        )
        assert result.IsReceiving is True

    @pytest.mark.asyncio
    async def test_with_expected_type_builds_type_str(self, parser):
        from pysui.sui.sui_pgql.pgql_types import OpenMoveDatatypeBodyGQL
        expected_type = MagicMock(spec=OpenMoveDatatypeBodyGQL)
        expected_type.package = "0x2"
        expected_type.module = "coin"
        expected_type.type_name = "Coin"
        result = await parser.fetch_or_transpose_object(
            "0xdeadbeef", is_receiving=False, is_mutable=False, expected_type=expected_type
        )
        assert isinstance(result, bcs.UnresolvedObjectArg)
        assert result.TypeStr == "0x2::coin::Coin"

    @pytest.mark.asyncio
    async def test_object_arg_passthrough(self, parser):
        inner = MagicMock()
        arg = MagicMock(spec=bcs.ObjectArg)
        result = await parser.fetch_or_transpose_object(
            arg, is_receiving=False, is_mutable=False, expected_type=None
        )
        assert result is arg


# ===========================================================================
# UnResolvingArgParser — gRPC
# ===========================================================================

class TestUnResolvingArgParserGRPC:
    """Test the fixed UnResolvingArgParser for gRPC."""

    @pytest.fixture
    def parser(self):
        from pysui.sui.sui_grpc.pgrpc_txn_async_argb import UnResolvingArgParser
        client = MagicMock()
        return UnResolvingArgParser(client)
    ...
    async def test_with_expected_type_builds_type_str(self, parser):
        from pysui.sui.sui_pgql.pgql_types import OpenMoveDatatypeBodyGQL
        expected_type = MagicMock(spec=OpenMoveDatatypeBodyGQL)
        expected_type.package = "0x3"
        expected_type.module = "staking_pool"
        expected_type.type_name = "StakedSui"
        result = await parser.fetch_or_transpose_object(
            "0xdeadbeef", is_receiving=False, is_mutable=True, expected_type=expected_type
        )
        assert result.TypeStr == "0x3::staking_pool::StakedSui"
        assert result.RefType == 2  # MUT_REF
