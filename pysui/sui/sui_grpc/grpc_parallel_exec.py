...
"""gRPC parallel transaction execution."""
...
class GrpcParallelTransactionExecutor(_BaseParallelExecutor):
    """Executes transactions in parallel using gRPC transport.
    ...
    **addressBalance mode**:
    ...
    async def _execute_single(
        ...
        # Update per-tx gas coin cache from effects (coins mode)
        if common_effects.gas_object is not None and self._gas_mode == "coins":
            go = common_effects.gas_object
            try:
                await grpc_exec.update_gas_coins([go.object_id])
            except Exception as exc:
                grpc_par_txn_exc_logger.warning("Failed to update gas coin from effects: %s", exc)
                await grpc_exec.invalidate_gas_coins()
    ...
    async def _refill_coin_pool(self, n: int) -> list[GasCoin]:
        """Split n new gas coins from the gas owner's balance via gRPC.
        ...
        """
        grpc_par_txn_exc_logger.info(
            "grpc_parallel_exec: refilling pool with %d coins @ %d MIST each",
        ...
        grpc_par_txn_exc_logger.info(
            "grpc_parallel_exec: refill produced %d coins", len(new_coins)
        )
        return new_coins
