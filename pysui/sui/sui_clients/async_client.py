...
class SuiClient(ClientMixin):
    ...
    async def execute(
        ...
        if not builder.txn_required:
            ...
            return result
        return await self._multi_signed_execution(builder, additional_signatures)

    async def execute_no_sign(
    ...
    async def get_gas(
        ...
        :param fetch_all: Flag indicating to fetch all coins, defaults to False
            Max is set by RPC Provider (currently 50 for Sui MystenLab providers)
        ...
        return await self._get_coins_for_type(address=address, fetch_all=fetch_all)
    ...
    async def get_coin(
        ...
        """
        address = address or self.config.active_address
        return await self._get_coins_for_type(
        ...
        if result.is_ok():
            if result.result_data.next_cursor and fetch_all:
                ...
                while objects.next_cursor and objects.has_next_page:
                    ...
                    objects = handle_result(await self.execute(builder))
                    accumer.extend(objects.data)
    ...
    async def get_objects_for(
        ...
        if len(identifiers) > self.max_gets:
            ...
            result = await self.execute(GetMultipleObjects(object_ids=identifiers))
