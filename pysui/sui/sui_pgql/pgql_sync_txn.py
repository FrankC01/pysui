...
class SuiTransaction(txbase):
    ...
    @versionadded(version="0.97.0", reason="Support Mysten gas from account balance.")
    def _build_txn_data_with_account_gas(
        self,
        ...
        """
        _tcons = self.client.rpc_config().protocolConfigs.transaction_constraints
        ...
        _res = self.client.execute_query_node(with_node=qn.GetBasicCurrentEpochInfo())
        if _res.is_ok():
            _cei: pgql_type.BasicCurrentEpochInfoGQL = _res.result_data
            min_epoch_expiration = min_epoch_expiration or _cei.epoch_id
            ...
            if not gas_budget:
                _res = self.client.execute_query_node(
                ...
                if _res.is_ok():
                    c_cost: int = int(
                        _res.result_data.transaction_block.gas_effects["gasSummary"][
                            "computationCost"
                        ]
                    )
                    s_cost: int = int(
    ...
    def transaction_data_with_account_gas(
        self,
        *,
        min_epoch_expiration: Optional[int] = None,
        gas_budget: Optional[int] = None,
        ...
        """
        return self._build_txn_data_with_account_gas(min_epoch_expiration, gas_budget)

    def build_with_account_gas(
        self,
        *,
        min_epoch_expiration: Optional[int] = None,
        gas_budget: Optional[int] = None,
    ) -> str:
        """build_with_account_gas Creates the BCS TransactionData using account balance for gas, serialize to base64 string and return.
    ...
    def build_sign_with_account_gas(
        ...
        *,
        ...
        sigs = self.signer_block.get_signatures(
            config=self.client.config, tx_bytes=txn_bytes
        )
        return {self._BUILD_BYTE_STR: txn_bytes, self._SIG_ARRAY: sigs}
